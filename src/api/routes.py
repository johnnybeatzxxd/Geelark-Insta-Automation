...
def start_automation(selection: DeviceSelection):
    ...
    """
    update_data = {"task_mode": selection.mode, "is_enabled": True}
...
def list_accounts():
    ...
    accounts = list(Account.select().where(Account.status != "archived"))
...
def add_targets(targets: List[TargetBase]):
    ...
    data = [
        {"username": extract_username(t.username), "source": t.source} for t in targets
    ]
...
async def websocket_endpoint(websocket: WebSocket, device_id: str):
    ...
    for l in reversed(list(recent)):
        await websocket.send_json(
            {"timestamp": str(l.timestamp), "level": l.level, "message": l.message}
        )
