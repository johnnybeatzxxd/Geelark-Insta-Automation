...
class Account(BaseModel):
    ...
    runtime_status = CharField(default="READY")
...
class SystemCommand(BaseModel):
    ...
    target_id = CharField(
        null=True
    )  # device_id (e.g. 'margieserra96') or NULL for global
...
def initialize_db():
    ...
    if os.getenv("DB_HOST"):
        ...
        for i in range(10):
            try:
                print(
                    f"[DB] Attempting connection to Postgres at {DB_HOST} ({i + 1}/10)..."
                )
                db.init(
                    DB_NAME, user=DB_USER, password=DB_PASS, host=DB_HOST, port=5432
                )
                db.connect()
    ...
    # Create Tables
    db.create_tables(
        [SystemConfig, Account, Target, Action, SystemCommand, DeviceLog], safe=True
    )

    # Initialize Defaults
    SystemConfig.get_or_create(
        key="global_automation_status", defaults={"value": "OFF"}
    )
...
def sync_devices_with_api(api_devices: List[Dict]):
    ...
    if api_ids:
        ...
        print(
            "[DB] API returned 0 devices. Skipping archival to prevent accidental lockout."
        )
...
def complete_command(cmd_id, status="completed"):
    SystemCommand.update(status=status, executed_at=datetime.datetime.now()).where(
        SystemCommand.id == cmd_id
    ).execute()
...
def set_global_automation(state: bool):
    ...
    SystemConfig.update(value=val).where(
        SystemConfig.key == "global_automation_status"
    ).execute()
...
def update_session_config(new_config: Dict):
    ...
    SystemConfig.insert(key="session_config", value=val).on_conflict(
        conflict_target=[SystemConfig.key], preserve=[SystemConfig.value]
    ).execute()
...
def get_runnable_accounts() -> List[Account]:
    ...
    return list(
        Account.select()
        .where((Account.status == "active") & (Account.is_enabled == True))
...
def import_targets_from_file(
    filepath: str = "targets.txt", source_tag: str = "manual_import"
) -> int:
...
def get_account_heat_stats(account_id: str):
    ...
    return {"recent_2h": recent_count, "rolling_24h": daily_rolling_count}
...
def reserve_targets(account_id: int, limit: int) -> List[str]:
    ...
    with db.atomic():
        # Select candidates
        candidates = Target.select().where(Target.status == "pending").limit(limit)
        ...
        (
            ...
            .where(Target.username.in_(reserved_usernames))
            .execute()
        )
...
def log_action(static_device_id: str, target_username: str, result: str):
    ...
    try:
        ...
        final_status = (
            "completed"
            if result in ["success", "already_following", "requested"]
            else "failed"
        )
        ...
        # 4. Create Action Log
        Action.create(account=acc, target=tgt, action_type="follow", result=result)
...
def release_reserved_targets(device_id):
    ...
    rows_updated = (
        ...
        .where((Target.reserved_by == device_id) & (Target.status == "reserved"))
        .execute()
    )
...
def update_account_runtime_status(device_id, status):
    Account.update(runtime_status=status).where(
...
def set_account_cooldown(device_id: str, hours: float = 2.0):
    ...
    Account.update(cooldown_until=cooldown_end).where(
        Account.device_id == device_id
    ).execute()
    return cooldown_end
...
def configure_and_enable_accounts(
    device_ids: List[str], mode: str, warmup_day: Optional[int] = None
):
    """
    ...
    (Account.update(update_data).where(Account.device_id.in_(device_ids)).execute())
